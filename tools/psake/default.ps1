Properties {
	...
	$solution_name = "$base_dir\WebApiContrib.Formatting.JavaScriptSerializer.sln"
	$test_dll = "$build_artifacts_dir\WebApiContrib.Formatting.JavaScriptSerializer.Tests.dll"
...
Task NuGetBuild {
	& $nuget_exe pack "$base_dir/src/WebApiContrib.Formatting.JavaScriptSerializer/WebApiContrib.Formatting.JavaScriptSerializer.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
